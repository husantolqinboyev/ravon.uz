...
export function DashboardNav({ user, onLogout }: DashboardNavProps) {
  ...
  const location = useLocation();
   const { canAccessTeacher, isLoading: roleLoading } = useUserRole();
 
   // Combine nav items based on role
   const visibleNavItems = canAccessTeacher 
     ? [...navItems.slice(0, 6), ...protectedNavItems, ...navItems.slice(6)]
     : navItems;
